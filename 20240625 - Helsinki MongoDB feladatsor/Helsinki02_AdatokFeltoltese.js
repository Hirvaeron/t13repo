...
function ObjektumFeltolto(vizsgaltTomb) {
    let ujAdatok = [];
    for (let i = 0; i < vizsgaltTomb.length; i++) {
        let ideiglenesAdatok = vizsgaltTomb[i].split(" ");
        let ujObjektum = {
            Helyezes: Number(ideiglenesAdatok[0]),
            CsapatMeret: Number(ideiglenesAdatok[1]),
            Sportag: ideiglenesAdatok[2],
            VersenySzam: ideiglenesAdatok[3]
        }
...
async function AdatokFeltoltese() {
    try {
        ...
        let VersenyAdatok = ObjektumFeltolto(eredmenyek);
        const muveletek = await collection.insertMany(VersenyAdatok);
