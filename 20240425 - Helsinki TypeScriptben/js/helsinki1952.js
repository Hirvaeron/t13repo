...
var helsinkiObjektum = ObjektumFeltolto(helsinki);
...
function MegszerzettErmekSzamaOsszesen(ermek) {
    return ermek[0] + ermek[1] + ermek[2];
}
function ErmeAdatokKiir(ermeKategoriak, osszesErem) {
...
var ermeAdatok = MegszerzettErmekSzama(helsinkiObjektum);
ErmeAdatokKiir(ermeAdatok, MegszerzettErmekSzamaOsszesen(ermeAdatok));
function OlimpiaiPontokSzama(vizsgaltTomb) {
    var osszpontszam = 0;
    for (var i = 0; i < vizsgaltTomb.length; i++) {
...
function OlimpiaiPontokSzamaKiir(eredmeny) {
...
function UszasEsTornaSzama(vizsgaltTomb) {
    var uszas = 0;
    var torna = 0;
    for (var i = 0; i < vizsgaltTomb.length; i++) {
...
function UszasEsTornaVizsgalat(ermekSzama) {
    var uszasErmekSzama = ermekSzama[0];
    var tornaErmekSzama = ermekSzama[1];
...
function UszasEsTornaVizsgalatKiir(vizsgalatEredmenye) {
    document.write("<br>6. feladat: <br>");
    document.write(vizsgalatEredmenye);
}
var uszasEsTornaErmekSzama = UszasEsTornaSzama(helsinkiObjektum);
UszasEsTornaVizsgalatKiir(UszasEsTornaVizsgalat(uszasEsTornaErmekSzama));
function TypoJavitas(vizsgaltTomb) {
    for (var i = 0; i < vizsgaltTomb.length; i++) {
...
function TombGeneralo(vizsgaltTomb) {
    var helsinki2 = [];
    for (var i = 0; i < vizsgaltTomb.length; i++) {
        var _a = vizsgaltTomb[i], helyezes = _a.helyezes, sportoloSzama = _a.sportoloSzama, sportag = _a.sportag, versenyszam = _a.versenyszam;
        var row = "".concat(helyezes, " ").concat(sportoloSzama, " ").concat(sportag, " ").concat(versenyszam);
        helsinki2.push(row);
    }
    return helsinki2;
}
...
function LegtobbSportolo(vizsgaltTomb) {
    var legtobbSportoloSzam = vizsgaltTomb[0].sportoloSzama;
    var maxIndex = 0;
    for (var i = 0; i < vizsgaltTomb.length; i++) {
...
function LegtobbSportoloKiir(kiirandoAdatok) {
