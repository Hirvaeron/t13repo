...
interface KosarlabdaEredmenyek {
    hazaiCsapatNeve: string;
    idegenCsapatNeve: string;
    hazaiCsapatPont: number;
    idegenCsapatPont: number;
    merkozesHelyszine: string;
    merkozesIdopontja: string;
}

function ObjektumFeltolto(feltoltendoElem: string[]): KosarlabdaEredmenyek[] {
    let beolvasottAdatok: KosarlabdaEredmenyek[] = [];
    for (let i: number = 0; i < feltoltendoElem.length; i++) {
        let daraboltAdatok = feltoltendoElem[i].split(";");
        let merkozesAdatok: KosarlabdaEredmenyek = {
            hazaiCsapatNeve: daraboltAdatok[0],
            idegenCsapatNeve: daraboltAdatok[1],
            hazaiCsapatPont: Number(daraboltAdatok[2]),
            idegenCsapatPont: Number(daraboltAdatok[3]),
            merkozesHelyszine: daraboltAdatok[4],
            merkozesIdopontja: daraboltAdatok[5]
        }
        beolvasottAdatok.push(merkozesAdatok);
    }
    return beolvasottAdatok;
}
let kosarlabdaObjektum = ObjektumFeltolto(eredmenyek);
console.log(kosarlabdaObjektum);

function RealMadridMerkozesSzam(vizsgaltTomb: KosarlabdaEredmenyek[]): [number, number] {
    let hazaiMeccsekSzama: number = 0;
    let idegenMeccsekSzama: number = 0;
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (vizsgaltTomb[i].hazaiCsapatNeve == "Real Madrid") {
            hazaiMeccsekSzama++;
        }
        if (vizsgaltTomb[i].idegenCsapatNeve == "Real Madrid") {
            idegenMeccsekSzama++;
        }
    }
    return [hazaiMeccsekSzama, idegenMeccsekSzama];
}

function RealMadridMerkozesSzamKiir(MerkozesekSzama: [number, number]): void {
    document.write(`3. feladat: Real Madrid: Hazai: ${MerkozesekSzama[0]}, Idegen: ${MerkozesekSzama[1]}`);
}

RealMadridMerkozesSzamKiir(RealMadridMerkozesSzam(kosarlabdaObjektum));

function DontetlenVizsgalat(vizsgaltTomb: KosarlabdaEredmenyek[]): boolean {
    let vizsgalatEredmenye = false;
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (vizsgaltTomb[i].hazaiCsapatPont == vizsgaltTomb[i].idegenCsapatPont) {
            vizsgalatEredmenye = true;
        }
    }
    return vizsgalatEredmenye;
}

function DontetlenVizsgalatKiir(vizsgalatEredmeny: boolean): void {
    document.write("<br>4. feladat: Volt döntetlen? ")
    if (vizsgalatEredmeny) {
        document.write("igen");
    } else {
        document.write("nem");
    }
...
DontetlenVizsgalatKiir(DontetlenVizsgalat(kosarlabdaObjektum));

function BarcelonaiCsapatNeve(vizsgaltTomb: KosarlabdaEredmenyek[]): string {
    let keresettSzoveg: string = "Barcelona";
    let pontosTalalat: string = "";
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        let aktualisHazaiCsapat = vizsgaltTomb[i].hazaiCsapatNeve;
        if (aktualisHazaiCsapat.includes(keresettSzoveg)) {
            pontosTalalat = aktualisHazaiCsapat;
        }
    }
    return pontosTalalat;
}

function BarcelonaiCsapatNeveKiir(csapatNeve: string): void {
    document.write(`<br>5. feladat: barcelonai csapat neve: ${csapatNeve}`);
}
BarcelonaiCsapatNeveKiir(BarcelonaiCsapatNeve(kosarlabdaObjektum));

interface MeccsAdatok {
    hazaiCsapatNeve: string;
    idegenCsapatNeve: string;
    hazaiCsapatPont: number;
    idegenCsapatPont: number;
...
function MeccsAdatFeltolto(vizsgaltTomb: KosarlabdaEredmenyek[]): MeccsAdatok[] {
    let beolvasottAdatok: MeccsAdatok[] = [];
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (vizsgaltTomb[i].merkozesIdopontja == "2004-11-21") {
            let merkozesAdatok: MeccsAdatok = {
                hazaiCsapatNeve: vizsgaltTomb[i].hazaiCsapatNeve,
                idegenCsapatNeve: vizsgaltTomb[i].idegenCsapatNeve,
                hazaiCsapatPont: vizsgaltTomb[i].hazaiCsapatPont,
                idegenCsapatPont: vizsgaltTomb[i].idegenCsapatPont
            }
            beolvasottAdatok.push(merkozesAdatok);
        }
    }
...
function MeccsAdatKiir(vizsgaltTomb: MeccsAdatok[]): void {
    document.write("<br>6. feladat:");
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        let aktualisAdat = vizsgaltTomb[i];
        document.write(`<br>${aktualisAdat.hazaiCsapatNeve}-${aktualisAdat.idegenCsapatNeve} (${aktualisAdat.hazaiCsapatPont}:${aktualisAdat.idegenCsapatPont})`);
    }
}
MeccsAdatKiir(MeccsAdatFeltolto(kosarlabdaObjektum));

interface StadionAdatok {
    stadionNeve: string;
    merkozesekSzama: number;
}

function StadionokListaja(vizsgaltTomb: KosarlabdaEredmenyek[]): string[] {
    let stadionokListaja: string[] = [];
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        let szerepelE: boolean = false;
        for (let j: number = 0; j < stadionokListaja.length; j++) {
            if (vizsgaltTomb[i].merkozesHelyszine == stadionokListaja[j]) {
                szerepelE = true;
                break;
            }
        }
        if (!szerepelE) {
            stadionokListaja.push(vizsgaltTomb[i].merkozesHelyszine);
        }
    }
    return stadionokListaja;
}

console.log(StadionokListaja(kosarlabdaObjektum));

function StadionSzamlalo(vizsgaltTomb: KosarlabdaEredmenyek[], keresettStadion: string): number {
    let db: number = 0;
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (keresettStadion == vizsgaltTomb[i].merkozesHelyszine) {
            db++;
        }
    }
    return db;
...
console.log(StadionSzamlalo(kosarlabdaObjektum, StadionokListaja(kosarlabdaObjektum)[1]));

function StadionAdatFeltolto(stadionLista: string[]): StadionAdatok[] {
    let beolvasottAdatok: StadionAdatok[] = [];
    for (let i: number = 0; i < stadionLista.length; i++) {
        let stadionAdatok: StadionAdatok = {
            stadionNeve: stadionLista[i],
            merkozesekSzama: StadionSzamlalo(kosarlabdaObjektum, stadionLista[i])
        }
        beolvasottAdatok.push(stadionAdatok);
    }
    return beolvasottAdatok;
}
let stadionokMerkozesSzamai = StadionAdatFeltolto(StadionokListaja(kosarlabdaObjektum));

function StadionAdatKiir(vizsgaltTomb: StadionAdatok[]): void {
    document.write("<br>7. feladat:");
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (vizsgaltTomb[i].merkozesekSzama > 20) {
            document.write(`<br>${vizsgaltTomb[i].stadionNeve}: ${vizsgaltTomb[i].merkozesekSzama}`);
        }
    }
}

StadionAdatKiir(stadionokMerkozesSzamai);